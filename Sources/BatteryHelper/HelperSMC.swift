...
enum HelperSMCError: LocalizedError {
    case driver
    case notPrivileged
    case failed
    ...
    var errorDescription: String? {
        switch self {
        case .driver: "AppleSMC not found"
        case .notPrivileged: "Not privileged"
        case .failed: "SMC call failed"
        }
    }
}

struct HelperSMCParam {
    var key: UInt32 = 0
...
final class HelperSMC: @unchecked Sendable {
    ...
    private func readInfo(_ name: String) throws -> UInt32 {
        var input = HelperSMCParam()
        input.key = fourChar(name)
        input.data8 = 9
        return try call(&input).dataSize
    }
    ...
    private func call(_ input: inout HelperSMCParam) throws -> HelperSMCParam {
        guard open() else { throw HelperSMCError.driver }
        var output = HelperSMCParam()
        let stride = MemoryLayout<HelperSMCParam>.stride
        var outputSize = stride
        let kr = IOConnectCallStructMethod(connection, 2, &input, stride, &output, &outputSize)
        if kr == kIOReturnNotPrivileged { throw HelperSMCError.notPrivileged }
        if kr != KERN_SUCCESS || output.result != 0 { throw HelperSMCError.failed }
        return output
    }

    private func fourChar(_ name: String) -> UInt32 {
