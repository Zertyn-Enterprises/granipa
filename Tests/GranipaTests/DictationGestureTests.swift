    ...
    struct HotkeyDispatchTests {
        @Test @MainActor func modifierHandlersReceivePhysicalEventTimestamp() async throws {
            ...
            HotkeyManager.shared.bindForTesting(
                id: id,
            ...
            #expect(down.keyCode == UInt16(kVK_RightCommand))
            #expect(down.modifierFlags.rawValue & UInt(NX_DEVICERCMDKEYMASK) != 0)
            #expect(
                HotkeyBinding.eventReportsModifierDown(
                    keyCode: UInt32(kVK_RightCommand), flags: down.modifierFlags))
            #expect(up.modifierFlags.rawValue & UInt(NX_DEVICERCMDKEYMASK) == 0)

            HotkeyManager.shared.handleModifierEventForTesting(down)
            HotkeyManager.shared.handleModifierEventForTesting(up)
        ...
        @Test @MainActor func modifierIgnoresDuplicateDownUpAndAutorepeat() async throws {
            ...
            HotkeyManager.shared.handleModifierEventForTesting(down)
            HotkeyManager.shared.handleModifierEventForTesting(downRepeat)
            HotkeyManager.shared.handleModifierEventForTesting(up)
            HotkeyManager.shared.handleModifierEventForTesting(upAgain)
            let received = await waitUntil(timeout: .milliseconds(800)) {
        ...
        @Test @MainActor func carbonHotkeyIgnoresRepeatAndUnmatchedRelease() {
            ...
            HotkeyManager.shared.dispatchHotKeyForTesting(
                id: id, released: false, timestamp: 1.0)
            HotkeyManager.shared.dispatchHotKeyForTesting(
                id: id, released: false, timestamp: 1.02)
            HotkeyManager.shared.dispatchHotKeyForTesting(
                id: id, released: true, timestamp: 1.4)
            HotkeyManager.shared.dispatchHotKeyForTesting(
                id: id, released: true, timestamp: 1.5)
