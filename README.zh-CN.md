...
<p align="center">
  <img src="docs/transcript.png" width="760" alt="录制过程中的实时转写">
</p>
...
- **自动语言检测** — 开始录制的前几秒并行运行英语和西班牙语模型，自动保留正确的那个。
...
- **屏幕文字识别 / OCR**（`⌥⇧T`）— 框选任意屏幕区域，文字直接进入剪贴板（Vision 框架，支持中西英环境下的西班牙语 + 英语识别）。
...
```sh
swift build         # 编译
swift test          # 57 个测试：存储、API、webhooks、语言检测、窗口布局……
