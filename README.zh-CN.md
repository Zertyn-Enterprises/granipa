...
**默认本地。** 录音、实时转写和听写都在你的 Mac 上进行，除非你主动开启云引擎（Muse）或通过已有的 AI CLI 发送笔记。

> 这是 Granola（每月 14 美元）、Raycast 剪贴板历史、TextSniper 和 Rectangle 的个人自建替代品 — 集成在一个原生应用里。

> 当前公开版本是 **[2.0.0](../../releases/tag/v2.0.0)**（已公证）。完整功能说明以 [English README](README.md) 为准。发现问题？[提交 issue](../../issues)。
...
- 无遥测、无分析、无账户。唯一的后台网络请求是向 GitHub Releases 检查更新（Sparkle），且可以关闭。
...
- **Ask your notes** — 与单场会议或全部会议档案对话，由你的本地 AI 命令行工具驱动。
- **⌥Space 命令面板** — 会议、剪贴板、片段和操作集于一个启动器。
- 自定义快捷键 · 浅色模式 · 任意语言的音频语言识别。
- **Windows 支持** — [在这里投票和讨论](../../issues/11)。
...
**如何连接我的 Claude / ChatGPT / Gemini / Grok 账户？需要 API key 吗？**
完全不需要 API key。Grañipa 调用各提供商的官方命令行工具，用你已付费的订阅进行认证：安装它（例如 `npm install -g @anthropic-ai/claude-code`），在终端运行一次，浏览器会自动打开完成登录。这次性登录之后一切都是自动的 — 凭证由 CLI 自己保存，Grañipa 永远不会接触到。设置 → AI 中有每个提供商的安装命令和 **Test** 按钮。

**窗口管理或剪贴板快捷键不起作用。**
其他应用占用了这些快捷键 — 退出 Rectangle（相同的 ⌃⌥ 方案）或检查 Raycast 的自定义快捷键，然后重启 Grañipa。自动粘贴和窗口管理还需要辅助功能权限。

**如何更新应用？**
从 release 安装的：应用会检查 GitHub Releases 并提供“安装并重启”（菜单栏 → Check for Updates…）。从源码构建的：`git pull && ./Scripts/bundle.sh release`。你的会议数据存放在应用之外，更新永远不会影响它们。
